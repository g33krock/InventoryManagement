...
class Transaction(Base):
    ...
    customer = relationship(
        "User", foreign_keys=[customer_id], backref="customer_transactions"
    )
    employee = relationship(
        "User", foreign_keys=[employee_id], backref="employee_transactions"
    )
...
def seed_inventory(session):
    inventory_to_add = [
        ...
        {
            "name": "Double Cooler Tube",
            "price": 30.0,
            "rental": 4.0,
            "description": "River tube for 2 rides with an embedded cooler",
        },
    ]
